...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html(
        f"🏆 <b>ЧМ-2026 | Прогнозы чемпионата</b>\n\n"
...
async def cmd_predict(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Показываем только матчи текущего раунда
...
async def ask_match(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(active)
    msg = (
        f"⚽ <b>Матч {m['id']} ({idx+1}/{total})</b> — {m['date']}\n"
        f"<b>{m['home']} — {m['away']}</b>\n\n"
...
async def finish_prediction(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(ctx.user_data.get("active_matches", MATCHES))
    reply = f"🎉 <b>Прогноз сохранён!</b> ({len(pending)}/{total} матчей)\n\n" + "\n".join(lines)
